patches-own [
  feromona             ;; amount of feromona on this patch
  comida                 ;; amount of comida on this patch (0, 1, or 2)
  nido?                ;; true on nido patches, false elsewhere
  nido-olor           ;; number that is higher closer to the nido
  comida-source-number   ;; number (1, 2, or 3) to identify the comida sources
]
...
to setup
  ...
  [ set size 2         ;; easier to see
    set color red  ]   ;; red = not carrying comida
  setup-patches
  reset-ticks
end
...
to setup-nido  ;; patch procedure
  ;; set nido? variable to true inside the nido, false elsewhere
  set nido? (distancexy 0 0) < 5
  ;; spread a nido-olor over the whole world -- stronger near the nido
  set nido-olor 200 - distancexy 0 0
end

to setup-comida  ;; patch procedure
  ;; setup comida source one on the right
  if ((distancexy (0.6 * max-pxcor) 0) < 5) [
    set comida-source-number 1
  ]

  ;; setup comida source two on the lower-left
  if ((distancexy (-0.6 * max-pxcor) (-0.6 * max-pycor)) < 5)[
    set comida-source-number 2
  ]

  ;; setup comida source three on the upper-left
  if ((distancexy (-0.8 * max-pxcor) (0.8 * max-pycor)) < 5) [
    set comida-source-number 3
  ]

  ;; set "comida" at sources to either 1 or 2, randomly
  if (comida-source-number > 0) [
    set comida one-of [1 2]
  ]
end

to recolor-patch  ;; patch procedure
  ;; give color to nido and comida sources
  ifelse nido?
  [ set pcolor violet ]
  [ ifelse comida > 0
    [
      if comida-source-number = 1 [ set pcolor cyan ]
      if comida-source-number = 2 [ set pcolor sky  ]
      if comida-source-number = 3 [ set pcolor blue ]
    ]

    ;; scale color to show feromona concentration
    [ set pcolor scale-color green feromona 0.1 5 ]
  ]
end
...
to go  ;; forever button
  ...
  [ if who >= ticks [ stop ] ;; delay initial departure
    ifelse color = red
    [ look-for-comida ]       ;; not carrying comida? look for it
    [ return-to-nido ]       ;; carrying comida? take it back to nido
    wiggle
    fd 1
  ]

  diffuse feromona (diffusion-rate / 100)
  ask patches
  [
    set feromona feromona * (100 - evaporation-rate) / 100  ;; slowly evaporitmo feromona
    recolor-patch
  ]
...
to look-for-comida  ;; turtle procedure
  if comida > 0
  [ set color orange + 1     ;; pick up comida
    set comida comida - 1        ;; and reduce the comida source
    rt 180                   ;; and turn around
    stop ]
  ;; go in the direction where the feromona smell is strongest
  if (feromona >= 0.05) and (feromona < 2)
  [ ascendente-feromona ]
end

to return-to-nido  ;; turtle procedure
  ifelse nido?
  [ ;; drop comida and head out again
    set color red
    rt 180 ]
  [ set feromona feromona + 60  ;; drop some feromona
    ascendente-nido-olor ]         ;; head toward the greatest value of nido-olor
end

;; sniff left and right, and go where the strongest smell is
to ascendente-feromona  ;; turtle procedure
  let olor-ahead feromona-olor-at-angle   0
  let olor-right feromona-olor-at-angle  45
  let olor-left  feromona-olor-at-angle -45
  if (olor-right > olor-ahead) or (olor-left > olor-ahead)
  [ ifelse olor-right > olor-left
    [ rt 45 ]
    [ lt 45 ] ]
end

;; sniff left and right, and go where the strongest smell is
to ascendente-nido-olor  ;; turtle procedure
  let olor-ahead nido-olor-at-angle   0
  let olor-right nido-olor-at-angle  45
  let olor-left  nido-olor-at-angle -45
  if (olor-right > olor-ahead) or (olor-left > olor-ahead)
  [ ifelse olor-right > olor-left
    [ rt 45 ]
    [ lt 45 ] ]
...
to-report nido-olor-at-angle [angle]
  let p patch-right-and-ahead angle 1
  if p = nobody [ report 0 ]
  report [nido-olor] of p
end

to-report feromona-olor-at-angle [angle]
  let p patch-right-and-ahead angle 3
  if p = nobody [ report 0 ]
  report [feromona] of p
end
